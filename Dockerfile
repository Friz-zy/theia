...
# CMD ["npm", "run", "start:browser", "--", "--hostname=0.0.0.0", "--port=3000"] # still use localhost
WORKDIR /theia/examples/browser
CMD npx theia start /theia/workspace --hostname=0.0.0.0 --port 3000 --plugins=local-dir:/theia/plugins --ovsx-router-config=../ovsx-router-config.json
